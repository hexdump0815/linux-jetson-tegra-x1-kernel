...
./scripts/config --set-str CONFIG_LOCALVERSION "-bsp-txn"

for i in `cat /compile/doc/jetson-tx/misc.txn/options/additional-options-*-yes.txt`; do
  echo $i
  ./scripts/config -e $i
done

for i in `cat /compile/doc/jetson-tx/misc.txn/options/additional-options-*-mod.txt`; do
  echo $i
  ./scripts/config -m $i
done
